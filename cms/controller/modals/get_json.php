...
foreach ($modals as $index => $item) {
    ...
    if ($modal_items) {
        foreach ($modal_items as $value) {
            ...
            // }
            $active_en = $value["title_eng"] != '' || $value["text_eng"] != '' ? 'active' : '';
            $active_ar = $value["title_eng"] == '' && $value["text_eng"] == '' ? 'active' : '';
            if ($value['media_id'] != '' && $value['media_id'] != 0) {
                ...
                if ($media != false) {
                    ...
                    array_push($medias, '<div class="item"><video class="new_inner_img"><source src="#" type="#"></video><div class="box_content_innerrr english ' . $active_en . '"><h3>' . $value["title_eng"] . '</h3><p>' . html_entity_decode($value["text_eng"]) . '</p></div><div class="box_content_innerrr arabic ' . $active_ar . '"><h3>' . $value["title_ar"] . '</h3><p>' . html_entity_decode($value["text_ar"]) . '</p></div></div>');
                }
            } else {
                array_push($medias, '<div class="item"><video class="new_inner_img"><source src="#" type="#"></video><div class="box_content_innerrr english ' . $active_en . '"><h3>' . $value["title_eng"] . '</h3><p>' . html_entity_decode($value["text_eng"]) . '</p></div><div class="box_content_innerrr arabic ' . $active_ar . '"><h3>' . $value["title_ar"] . '</h3><p>' . html_entity_decode($value["text_ar"]) . '</p></div></div>');
            }
