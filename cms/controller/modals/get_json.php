        ...
        foreach ($modal_items as $value) {
            ...
            if ($value['media_id'] != '' && $value['media_id'] != 0) {
                ...
                if ($media != false) {
                    ...
                    if ($media['type'] == 'video') {
                        array_push($medias, '<div class="item"><video class="new_inner_img" autoplay controls onplay="pauseModalSlider(\'#modal' . $i . '\');" onended="playModalSlider(\'#modal' . $i . '\');" ' . $loop . '><source src="' . $items_config["modal_media_url"] . $media["name"] . '" type="' . $media["filetype"] . '"></video><div class="box_content_innerrr arabic ' . $active_ar . '"><h3>' . $value["title_ar"] . '</h3><p>' . html_entity_decode($value["text_ar"]) . '</p></div><div class="box_content_innerrr english ' . $active_en . '"><h3>' . $value["title_eng"] . '</h3><p>' . html_entity_decode($value["text_eng"]) . '</p></div></div>');
                    } else {
