        ...
        foreach ($modal_items as $value) {
            if ($value['media_id'] != '' && $value['media_id'] != 0) {
                ...
                if ($media != false) {
                    ...
                    array_push($medias, '<div class="item"><video class="new_inner_img"><source src="#" type="#"></video><div class="box_content_innerrr english active"><h3>' . $value["title_eng"] . '</h3><p>' . html_entity_decode($value["text_eng"]) . '</p></div><div class="box_content_innerrr arabic"><h3>' . $value["title_ar"] . '</h3><p>' . html_entity_decode($value["text_ar"]) . '</p></div></div>');
                }
            } else {
                array_push($medias, '<div class="item"><video class="new_inner_img"><source src="#" type="#"></video><div class="box_content_innerrr english active"><h3>' . $value["title_eng"] . '</h3><p>' . html_entity_decode($value["text_eng"]) . '</p></div><div class="box_content_innerrr arabic"><h3>' . $value["title_ar"] . '</h3><p>' . html_entity_decode($value["text_ar"]) . '</p></div></div>');
            }
