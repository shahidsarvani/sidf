...
if($tokens) {
    foreach ($tokens as $index => $value) {
        ...
        $temp .= '<div class="col-6 order-' . $value['sort_order'] . ' d-flex align-items-center justify-content-center flex-column next_tab logo_' . $i . '" data-slug="' . $value['slug'] . '" data-card_id="' . $value['rfid_card_id2'] . '" data-loader_name="' . (isset($loader_video) ? $items_config['rfid_loadermedia_url'] . $loader_video['name'] : "") . '" data-loader_type="' . (isset($loader_video) ? $loader_video['filetype'] : "") . '">
...
// die();
$filename = BASE_PATH . '/tokens2.json';
$fp = fopen($filename, 'w');
